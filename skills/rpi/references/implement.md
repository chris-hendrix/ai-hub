...
Execute a plan's checklist. `implement all <plan>` is the default; name a phase only to resume or rerun a specific one. With no path, use the most recent `.rpi/plans/*.md` and confirm with the user. The orchestrator owns git: create the branch per the plan's Branch & Commit Strategy and commit after each verified task — subagents never touch git.
...
3. **Verify & mark** — re-run the task's CHECK command yourself, then `rpi done <plan> <task-id>`. The orchestrator flips the checkbox, never the subagent — state advances only on verified results, not self-reports.
